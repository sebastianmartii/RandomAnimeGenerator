...
interface MainInfoDao {
    ...
    @Query("SELECT * FROM library_table WHERE library_type = :type AND is_favorite = 1")
    fun getAll(type: String): Flow<List<MainInfoEntity>>

    @Query("SELECT * FROM library_table WHERE library_type = :type AND is_favorite = 1 AND library_status = :libraryStatus")
