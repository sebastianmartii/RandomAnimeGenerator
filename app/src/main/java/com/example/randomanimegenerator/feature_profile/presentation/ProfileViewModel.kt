...
class ProfileViewModel @Inject constructor(
    ...
    val state = combine(_state, _animeEntriesCount, _mangaEntriesCount) { state, animeEntriesCount, mangaEntriesCount ->
        state.copy(
            animeEntriesCount = listOf(
                EntriesCount(LibraryStatus.ALL, animeEntriesCount.size),
                EntriesCount(LibraryStatus.PLANNING, animeEntriesCount.filter { it == LibraryStatus.PLANNING.toStatusString() }.size),
                EntriesCount(LibraryStatus.FINISHED, animeEntriesCount.filter { it == LibraryStatus.FINISHED.toStatusString() }.size),
                EntriesCount(LibraryStatus.WATCHING, animeEntriesCount.filter { it == LibraryStatus.WATCHING.toStatusString() }.size),
                EntriesCount(LibraryStatus.READING, animeEntriesCount.filter { it == LibraryStatus.READING.toStatusString() }.size),
                EntriesCount(LibraryStatus.PAUSED, animeEntriesCount.filter { it == LibraryStatus.PAUSED.toStatusString() }.size),
            ),
            mangaEntriesCount = listOf(
                EntriesCount(LibraryStatus.ALL, mangaEntriesCount.size),
                EntriesCount(LibraryStatus.PLANNING, mangaEntriesCount.filter { it == LibraryStatus.PLANNING.toStatusString() }.size),
                EntriesCount(LibraryStatus.FINISHED, mangaEntriesCount.filter { it == LibraryStatus.FINISHED.toStatusString() }.size),
                EntriesCount(LibraryStatus.WATCHING, mangaEntriesCount.filter { it == LibraryStatus.WATCHING.toStatusString() }.size),
                EntriesCount(LibraryStatus.READING, mangaEntriesCount.filter { it == LibraryStatus.READING.toStatusString() }.size),
                EntriesCount(LibraryStatus.PAUSED, mangaEntriesCount.filter { it == LibraryStatus.PAUSED.toStatusString() }.size),
            ),
        )
    }
